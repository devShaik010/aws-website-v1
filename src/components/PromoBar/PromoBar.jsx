...
const items = [
  ...
  '10th June 2026  ·  MJCET Hyderabad',
...
const PromoBar = () => (
  <Link
    ...
    className="promo-bar group fixed top-0 left-0 right-0 z-[4950] h-9 block overflow-hidden bg-black/85 backdrop-blur-md border-b border-violet-500/20 hover:border-violet-500/40 transition-colors duration-300"
  >
    {/* Soft warm vignette behind the marquee */}
    <div className="absolute inset-0 bg-gradient-to-r from-[#0d0820] via-black to-[#0d0820] opacity-90 pointer-events-none" />
    {/* Accent hairline on top and bottom */}
    <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-violet-500/70 to-transparent pointer-events-none" />
      ...
      <div className="promo-marquee flex items-center whitespace-nowrap will-change-transform">
        {loop.map((item, idx) => (
          <span key={idx} className="flex items-center gap-4 px-5 shrink-0">
            <span className="w-1 h-1 rounded-full bg-violet-400 shadow-[0_0_8px_rgba(167,139,250,0.8)] shrink-0" />
